    ...
    switch (projectState?.project ?? message?.data?.project) {
      ...
      case ProjectName.connections: {
        if (message.command === ExtensionCommand.initializeConnectionFrame) {
          dispatch(initializeConnectionsDesigner({state: message.data, accessToken: message.data.azureDetails.accessToken ?? '', panelId: message.data.panelId, connections: message.data.connections}));
