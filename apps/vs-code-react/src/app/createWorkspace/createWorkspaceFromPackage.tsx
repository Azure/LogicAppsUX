...
import { PackageSetupStep, ReviewCreateStep } from './steps/';
...
export const CreateWorkspaceFromPackage: React.FC = () => {
  ...
  const intlText = {
    CREATE_WORKSPACE_FROM_PACKAGE: intl.formatMessage({
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 0: {
        ...
        const baseFieldsValid = packagePathValid && workspacePathValid && workspaceNameValid && logicAppTypeValid && logicAppNameValid;
  ...
  const isStepCompleted = (stepIndex: number) => {
    switch (stepIndex) {
      case 0: {
        ...
        const logicAppNameValid =
          logicAppName.trim() !== '' && logicAppNameValidation.test(logicAppName.trim()) && !isNameAlreadyInWorkspace(logicAppName.trim());
        const baseFieldsValid = packagePathValid && workspacePathValid && workspaceNameValid && logicAppNameValid;
  ...
  const handleCreate = () => {
    ...
    vscode.postMessage({ command: 'createWorkspaceFromPackage', data });
  };
  ...
  return (
    <div className={styles.createWorkspaceContainer}>
      <Text className={styles.createWorkspaceTitle} style={{ display: 'block' }}>
        {intlText.CREATE_WORKSPACE_FROM_PACKAGE}
      </Text>
