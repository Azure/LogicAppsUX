...
import { AppSettingsTreeItem, DeleteLastServicePlanStep, DeleteSiteStep, DeploymentsTreeItem, DeploymentTreeItem, getFile, IDeployContext, LogFilesTreeItem, ParsedSite, SiteFilesTreeItem } from '@microsoft/vscode-azext-azureappservice';
import { AzExtParentTreeItem, AzExtTreeItem, AzureWizard, DeleteConfirmationStep, IActionContext, nonNullValue, TreeItemIconPath } from '@microsoft/vscode-azext-utils';
...
import { FuncVersion, ILocalSettingsJson, latestGAVersion, ProjectSource } from '@microsoft-logic-apps/utils';
...
export abstract class SlotTreeItemBase extends AzExtParentTreeItem implements IProjectTreeItem {
    ...
    public async getApplicationSettings(context: IDeployContext): Promise<ApplicationSettings> {
        const localSettings: ILocalSettingsJson = await getLocalSettingsJson(context, path.join(context.effectiveDeployFsPath, localSettingsFileName));
    ...
    public async loadMoreChildrenImpl(_clearCache: boolean, context: IActionContext): Promise<AzExtTreeItem[]> {
        ...
        if (!this._artifactsTreeItem) {
            try {
                ...
                if (error.statusCode === 404) {
                    return [this._workflowsTreeItem, this.configurationsTreeItem, this._siteFilesTreeItem, this._logFilesTreeItem, this.deploymentsNode];
                }
            }
        ...
        return [this._workflowsTreeItem, this.configurationsTreeItem, this._artifactsTreeItem, this._siteFilesTreeItem, this._logFilesTreeItem, this.deploymentsNode];
