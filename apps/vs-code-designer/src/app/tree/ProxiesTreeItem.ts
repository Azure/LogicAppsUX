...
import type { ISiteFile } from '@microsoft/vscode-azext-azureappservice';
import type { AzExtTreeItem, IActionContext, TreeItemIconPath } from '@microsoft/vscode-azext-utils';
import { AzExtParentTreeItem, DialogResponses, parseError } from '@microsoft/vscode-azext-utils';

export class ProxiesTreeItem extends AzExtParentTreeItem {
  public static contextValue = 'azFuncProxies';
  ...
  public async loadMoreChildrenImpl(_clearCache: boolean, context: IActionContext): Promise<AzExtTreeItem[]> {
    ...
    try {
      ...
      this._proxyConfig = rawProxyConfig as IProxyConfig;
  ...
  public async deleteProxy(context: IActionContext, name: string): Promise<void> {
    ...
    if (this._deletingProxy) {
      throw new Error(
        localize(
          'multipleProxyOperations',
          'An operation on the proxy config is already in progress. Wait until it has finished and try again.'
        )
      );
    } else {
...
interface IRawProxyConfig {
  proxies?: { [name: string]: Record<string, any> };
}
