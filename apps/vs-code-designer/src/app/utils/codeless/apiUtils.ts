...
import { connectionsFileName, managementApiPrefix, parametersFileName, workflowAppApiVersion } from "apps/vs-code-designer/src/constants";
import type { SlotTreeItemBase } from "../../tree/SlotTreeItemBase";
import { sendAzureRequest } from "../requestUtils";
import * as vscode from 'vscode';
import { localize } from "../../../localize";
import { Artifacts, HTTP_METHODS, IArtifactFile, IConnectionsFileContent, IParametersFileContent, IWorkflowFileContent, Parameter } from "@microsoft-logic-apps/utils";
import { RemoteWorkflowTreeItem } from "../../tree/remoteProject/RemoteWorkflowTreeItem";
...
export async function getWorkflow(node: SlotTreeItemBase, workflow: RemoteWorkflowTreeItem, context: IActionContext): Promise<IWorkflowFileContent> {
...
export async function listWorkflows(node: SlotTreeItemBase, context: IActionContext): Promise<Object[]> {
    const url: string = `${node.id}/hostruntime${managementApiPrefix}/workflows?api-version=${workflowAppApiVersion}`;
    try {
        const response = await sendAzureRequest(url, context, "GET",  node.site.subscription);
        return response.parsedBody;
    } catch (error) {
