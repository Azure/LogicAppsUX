...
describe('projectFilesConsistency', () => {
  ...
  describe('validateDesignTimeDirectory', () => {
    ...
    it('reports valid settings for a Node design-time file for a codeful project (setting off)', async () => {
      // Codeful projects must run the Node design-time worker regardless of the workspace setting,
      // because the dotnet + in-process .NET 8 host loads and locks lib/codeful.
      vi.mocked(useNodeDesignTimeWorker).mockReturnValue(false);
    ...
    it('reports invalid settings for a dotnet + in-process .NET 8 file for a codeful project (must be Node)', async () => {
      // A codeful project left on the dotnet/net8 design-time host (e.g. generated by an older build)
      // must be treated as invalid so it self-heals to the Node worker on the next design-time start.
      vi.mocked(useNodeDesignTimeWorker).mockReturnValue(false);
      vi.mocked(hasCodefulSdkReference).mockResolvedValue(true);
      mockFiles({
        [designTimeDir]: '',
