...
export class CodelessWorkflowCreateStep extends WorkflowCreateStepBase<IFunctionWizardContext> {
  ...
  public static async create(context: IActionContext): Promise<CodelessWorkflowCreateStep> {
  ...
  public async executeCore(context: IFunctionWizardContext): Promise<string> {
    ...
    const workflowFolderPath: string = path.join(context.projectPath, nonNullProp(context, 'functionName'));
    const workflowFilePath: string = path.join(workflowFolderPath, workflowFileName);

    const workflowDefinition: StandardApp = getCodelessWorkflowTemplate(
      context.projectType,
      template?.id as WorkflowType,
    ...
    if (nonNullProp(context, 'workflowProjectType') === WorkflowProjectType.Nuget) {
      ...
      const dotnetVersion = await getFramework(context, workflowFolderPath);
