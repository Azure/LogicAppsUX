...
export class CreateFunctionAppFiles {
  ...
  private async createCsFile(
    ...
    const templatePath = path.join(getAssetsRoot(), this.templateFolderName[projectType], templateFile);
  ...
  private async createRulesFiles(functionFolderPath: string): Promise<void> {
    const csTemplatePath = path.join(getAssetsRoot(), 'RuleSetProjectTemplate', 'ContosoPurchase');
