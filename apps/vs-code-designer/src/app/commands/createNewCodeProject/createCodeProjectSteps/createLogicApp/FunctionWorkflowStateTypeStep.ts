...
import { CodelessFunctiontWorkflowCreateStep } from './CodelessFunctiontWorkflowCreateStep';
 ...
 */
export class FunctionWorkflowStateTypeStep extends AzureWizardPromptStep<IFunctionWizardContext> {
  ...
  ): Promise<FunctionWorkflowStateTypeStep> {
    return new FunctionWorkflowStateTypeStep(options.triggerSettings, options.isProjectWizard);
  }
  ...
  public async getSubWizard(context: IFunctionWizardContext): Promise<IWizardOptions<IFunctionWizardContext> | undefined> {
    ...
    if (template) {
      ...
      executeSteps.push(await CodelessFunctiontWorkflowCreateStep.createStep(context));
