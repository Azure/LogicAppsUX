...
export class CodelessFunctiontWorkflowCreateStep extends WorkflowCreateStepBase<IFunctionWizardContext> {
  ...
  public static async createStep(context: IActionContext): Promise<CodelessFunctiontWorkflowCreateStep> {
    // Ensure that the .NET Core SDK is installed on the user's machine
    await validateDotnetInstalled(context);
    return new CodelessFunctiontWorkflowCreateStep();
  }
