...
export class FunctionProjectWorkflow extends WorkflowCreateStepBase<IFunctionWizardContext> {
  // Private constructor to prevent direct instantiation of this class
  private constructor() {
    super();
  }

  // Static method that creates a new instance of the FunctionProjectWorkflow class and returns it
  public static async createStep(context: IActionContext): Promise<FunctionProjectWorkflow> {
    // Ensure that the .NET Core SDK is installed on the user's machine
    await validateDotnetInstalled(context);
    return new FunctionProjectWorkflow();
  }
