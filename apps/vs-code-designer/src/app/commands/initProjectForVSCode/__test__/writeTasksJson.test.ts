...
describe('InitProjectStepBase.writeTasksJson writes getTasks output', () => {
  const writeTasksJson = (InitProjectStepBase.prototype as any).writeTasksJson as (
    this: unknown,
    context: unknown,
    vscodePath: string
  ) => Promise<void>;
  const insertNewTasks = (InitProjectStepBase.prototype as any).insertNewTasks;
  ...
  it('writes the tasks returned by getTasks into tasks.json', async () => {
    ...
    const thisArg = {
      getTasks,
      insertNewTasks,
      addSubDir: async (_context: unknown, fsPath: string) => fsPath,
    };
