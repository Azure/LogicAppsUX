...
import { promptForLogicAppWithoutCustomCode } from '../../../utils/workspace';
...
export class LogicAppNameStep extends AzureWizardPromptStep<IProjectWizardContext> {
  public async prompt(context: IProjectWizardContext): Promise<void> {
    if (context.projectType === ProjectType.logicApp || context.shouldCreateLogicAppProject) {
      ...
      const logicAppFolder = await promptForLogicAppWithoutCustomCode(context);
  ...
  private async validateLogicAppName(name: string | undefined, context: IProjectWizardContext): Promise<string | undefined> {
    ...
    if (fse.existsSync(context.workspaceFilePath)) {
      const workspaceFileContent = await vscode.workspace.fs.readFile(vscode.Uri.file(context.workspaceFilePath));
