...
export default class DataMapperExt {
  ...
  public static createOrShow(
    ...
    if (ext.dataMapPanelManagers[dataMapName]) {
      ...
      window.showInformationMessage(
        `A Data Mapper panel is already open for this data map (${dataMapName}).`
      );
  ...
  */
  public static loadMapDefinition = (
    mapDefinitionString: string | undefined,
    ext: any
  ): MapDefinitionEntry => {
    const onWarning = (e: yaml.YAMLException) => {
      ext.showError(`Exception while parsing LML file!`, { detail: e.message}
      );
    };
    if (mapDefinitionString) {
      try {
        ...
        const mapDefinition = yaml.load(
          modifiedMapDefinitionString, { onWarning }
    ...
    for (const key in mapDefinition) {
      ...
      if (typeof curElement === "object" && curElement !== null) {
        if (Array.isArray(curElement)) {
          ...
          curElement.forEach((arrayElement) =>
            DataMapperExt.fixMapDefinitionCustomValues(arrayElement)
          );
        } else {
          DataMapperExt.fixMapDefinitionCustomValues(curElement);
        }
      } else if (
        Object.prototype.hasOwnProperty.call(mapDefinition, key) &&
        typeof curElement === "string"
      ) {
