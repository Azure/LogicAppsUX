...
export async function updateAzuriteConfiguration(context: IActionContext, configurationName: string, newValue: string): Promise<void> {
  const azuriteExtension = extensions.getExtension(azuriteExtensionId);

  if (azuriteExtension?.isActive) {
    const configuration = vscode.workspace.getConfiguration();
    await configuration.update(configurationName, newValue, vscode.ConfigurationTarget.Global);
  } else {
