...
export function CardFooter({ commentBox, connectionDisplayName, connectionRequired, staticResultsEnabled }: CardProps): JSX.Element | null {
  ...
  const badges = [
    ...
    ...(commentBox && commentBox.comment
      ? [{ title: COMMENT, content: commentBox.comment, iconProps: commentIconProps, active: true }]
      : []),
    ...(connectionRequired
      ? [{ title: connectionTitle, content: connectionDisplayName, iconProps: connectionIconProps, active: !!connectionDisplayName }]
      : []),
  ];
