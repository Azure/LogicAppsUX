...
test.describe(
  ...
  () => {
    test('Should collapse condition actions ', async ({ page }) => {
      ...
      // Check items inside conditions exist
      // True side
      ...
      // await expect(page.getByTestId('card-Increment variable 4').getByRole('button', { name: 'Increment variable' })).toBeVisible();
    ...
    test('Should collapse for each actions ', async ({ page }) => {
      ...
      // Check items inside foreach actions
