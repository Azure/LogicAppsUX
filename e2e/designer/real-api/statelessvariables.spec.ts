...
test.describe(
  ...
  () => {
    test('Variable Functionality works', async ({ page, request, browserName }) => {
      await page.goto('/');
      await GoToRealWorkflow(page, `wapp-lauxtest2${browserName}`, 'statelessVariables');
      ...
      await page
        ...
        .filter({ hasText: 'Namev1Add dynamic data or' })
        .first()
        .click();
      ...
      await page
        .getByTestId('rf__edge-Append_to_array_variable-Response')
        .getByRole('button', { name: 'Insert a new step between' })
        .first()
        .click();
