...
function generateHexId(text) {
  return crypto
    .createHash('sha512')
    .update(text)
    .digest('hex') // Convert to hex (base16)
    .slice(0, 8); // Use only first 8 characters
}
...
function updateFiles() {
  ...
  tsFiles.forEach((file) => {
    ...
    // Match and replace only inside intl.formatMessage({ id: "..." })
    content = content.replace(/intl\.formatMessage\s*\(\s*\{([^}]+)\}\s*\)/g, (match, messageContent) => {
      return messageContent.replace(/(id\s*:\s*)(["'`])([^"'`]+)\2/g, (idMatch, key, quote, oldId) => {
        if (!idMapping[oldId]) {
  ...
  const jsonFiles = glob.sync('src/locales/strings*.json');
