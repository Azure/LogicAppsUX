...
function cjs({ input, external, banner }) {
  return {
    ...
    plugins: [svelte(), commonJS(), babelPlugin, nodeResolve({ extensions: ['.ts', '.tsx'] })],
  };
}
...
function umdDev({ input, external, globals, banner, jsName }) {
  return {
    ...
    plugins: [svelte(), commonJS(), babelPlugin, nodeResolve({ extensions: ['.ts', '.tsx'] }), forceEnvPlugin('development')],
  };
}
