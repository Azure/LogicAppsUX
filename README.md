...
Graphify reports under each supported package's `src/graphify-out` directory describe core abstractions, communities, and cross-file dependencies. Read the relevant `GRAPH_REPORT.md` before exploring a large library.

```bash
# One-time CLI setup
pnpm run graphify:setup

# Rebuild all graphs or one library
pnpm run graphify:rebuild
pnpm run graphify:rebuild -- designer-v2

# Query a graph
graphify query "how does serialization work?" \
  --graph libs/designer-v2/src/graphify-out/graph.json
```

Graphs are regenerated by CI when supported TypeScript sources change on `main`; avoid copying graph statistics into documentation because they become stale as the codebase evolves.
