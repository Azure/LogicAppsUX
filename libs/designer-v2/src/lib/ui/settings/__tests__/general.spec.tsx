...
describe('ui/settings/sections/general', () => {
  ...
  it('should close the confirmation dialog without calling onConcurrencyToggle when nodeId changes', async () => {
    const user = userEvent.setup();
    const { props, rerender } = renderGeneral();

    await user.click(getConcurrencyToggle());

    expect(screen.getByRole('alertdialog', { name: concurrencyDialogTitle })).toBeInTheDocument();

    rerender(<General {...createProps({ nodeId: 'node-2', onConcurrencyToggle: props.onConcurrencyToggle })} />);
