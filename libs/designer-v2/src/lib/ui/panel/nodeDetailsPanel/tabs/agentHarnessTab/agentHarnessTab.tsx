 ...
 * For known single-argument functions (variables, parameters, triggerBody, etc.),
 * extracts just the argument name to match the token pill style in the Parameters tab.
 * e.g. "@{variables('documentContent')}" → "documentContent"
 *      "@triggerBody()" → "triggerBody()"
 *      "@body('action1')" → "body - action1"
 */
const getExpressionTitle = (expression: string): string => {
  ...
  // For single-argument functions like variables('name'), parameters('name') — extract just the name
  const singleArgMatch = inner.match(/^(variables|parameters)\('([^']+)'\)$/i);
  if (singleArgMatch) {
    return singleArgMatch[2];
  }
  // For action-referencing functions like body('action'), outputs('action') — show "fn - name"
 ...
 * matching the visual style of InputToken without requiring a Lexical editor context.
 */
