...
export const parseChatHistory = (
  ...
  toolResultCallback: (agentName: string, toolName: string, iteration: number, subIteration: number, mcpToolName?: string) => void,
...
const parseMessage = (
  ...
  switch (messageEntryType) {
    ...
    case AgentMessageEntryType.ToolResult: {
      ...
      const mcpToolName = message.toolResultsPayload?.toolResult?.mcpServerToolName;
