...
export class BtmSerializer {
  ...
  public serialize(map: MapDocument): string {
    ...
    for (const preference of map.scriptTypePrecedence || this.defaultScriptTypePrecedence()) {
      precedence.ele(this.scriptTypeElementName(preference.type)).att('Enabled', preference.enabled ? 'Yes' : 'No');
    }
    ...
    for (const page of map.pages) {
      ...
      for (const functoid of page.functoids) {
        ...
        if (functoid.tableLooping) {
          const tableElem = fElem
            .ele('TableLoopingData')
        ...
        if (functoid.scriptImplementations?.length) {
          ...
          for (const script of functoid.scriptImplementations) {
            const scriptElement = code.ele('Script').att('Language', this.scriptTypeElementName(script.type));
          ...
          const assemblyReferences = new Set(functoid.scriptImplementations.flatMap((script) => script.assemblyReferences || []));
  ...
  private parseScriptTypePrecedence(node: any): Array<{ type: ScriptType; enabled: boolean }> {
    if (!node) {
      return this.defaultScriptTypePrecedence();
    }
  ...
  private scriptTypeElementName(type: ScriptType): string {
    switch (type) {
      case ScriptType.InlineCSharp:
        return 'CSharp';
      case ScriptType.ExternalAssembly:
        return 'ExternalAssembly';
      case ScriptType.InlineVbNet:
        return 'VbNet';
      case ScriptType.InlineJScript:
        return 'JScript';
      case ScriptType.InlineXsltCallTemplate:
        return 'XsltCallTemplate';
      case ScriptType.InlineXslt:
        return 'Xslt';
      default:
        return 'CSharp';
    }
  ...
  private parsePages(pagesNode: any, scriptTypePrecedence: Array<{ type: ScriptType; enabled: boolean }>): MapPage[] {
    if (!pagesNode) {
  ...
  private parseLinks(page: any): MapLink[] {
    ...
    const linksNode = page['Links'] || page['links'];
    let links: any;
  ...
  private parseSourceLinkOption(value: string | undefined): SourceLinkOption {
    if (value === 'CopyName') {
      return SourceLinkOption.NameCopy;
    }
    if (value === 'CopyMixed') {
      return SourceLinkOption.MixedCopy;
    }
  ...
  private parseTargetLinkOption(value: string | undefined): TargetLinkOption {
    if (value === 'TopDown') {
      return TargetLinkOption.TopDown;
    }
    if (value === 'BottomUp') {
      return TargetLinkOption.BottomUp;
    }
  ...
  private parseFunctoids(page: any, scriptTypePrecedence: Array<{ type: ScriptType; enabled: boolean }>): MapFunctoid[] {
    ...
    const functoidsNode = page['Functoids'] || page['functoids'];
    let functoids: any;
    ...
    return fArray.map((f: any) => {
      ...
      const functoidId = Number.parseInt(f['@_Functoid-FID'] || f['@_TypeID'] || '0', 10);
      ...
      const rawX = Number.parseFloat(f['@_X-Cell'] || f['@_X'] || '0');
      const rawY = Number.parseFloat(f['@_Y-Cell'] || f['@_Y'] || '0');
      ...
      if (scriptType === ScriptType.ExternalAssembly) {
        ...
        const externalScript = Array.isArray(script)
          ? script.find((candidate: any) => candidate['@_Language']?.toLowerCase() === 'externalassembly')
          : script;
        if (externalScript) {
          parameters.push(
  ...
  private parseTableLoopingData(functoid: any): MapFunctoid['tableLooping'] {
    ...
    const rows = (Array.isArray(rawRows) ? rawRows : rawRows ? [rawRows] : []).map((row: any) => {
      ...
      return (Array.isArray(rawColumns) ? rawColumns : rawColumns ? [rawColumns] : []).map((column: any) => column['@_Value'] || '');
    });
    return {
      columns: Number.parseInt(table['@_Columns'] || '0', 10),
  ...
  private selectScript(
    ...
    for (const preference of precedence) {
      ...
      const script = scripts.find(
        (candidate: any) => !!candidate['@_Language'] && this.mapBizTalkScriptLanguage(candidate['@_Language']) === preference.type
      );
      if (script) {
        return {
  ...
  private parseScriptImplementations(f: any):
    | Array<{
        type: ScriptType;
        content?: string;
    ...
    return scripts
      ...
      .map((script: any) => ({
        ...
        isStatic: script['@_IsStatic'] ? script['@_IsStatic'].toLowerCase() === 'yes' : undefined,
      }));
  }
  ...
  private parseScriptType(f: any): ScriptType | undefined {
    ...
    if (scripterCode) {
      ...
      const script = Array.isArray(scripts)
        ? scripts.find((candidate: any) => candidate['@_Language']?.toLowerCase() === 'externalassembly') || scripts[0]
        : scripts;
      if (script) {
  ...
  private mapBizTalkScriptLanguage(lang: string): ScriptType {
    switch (lang.toLowerCase()) {
      case 'externalassembly':
        return ScriptType.ExternalAssembly;
      case 'csharp':
        return ScriptType.InlineCSharp;
      case 'vbnet':
        return ScriptType.InlineVbNet;
      case 'jscript':
        return ScriptType.InlineJScript;
      case 'xslt':
        return ScriptType.InlineXslt;
      case 'xsltcalltemplate':
        return ScriptType.InlineXsltCallTemplate;
      default:
        return ScriptType.InlineCSharp;
    }
  }
  ...
  private parseParameters(functoid: any): Array<{
    ...
    return paramArray.map((p: any, idx: number) => ({
      index: Number.parseInt(p['@_Index'] || String(idx), 10),
      value: p['@_Value'] || '',
  ...
  private parseParameterType(typeStr: string | undefined): ParameterType {
    ...
    switch (normalized) {
      ...
      case 'constant':
        return ParameterType.Constant;
      case 'scripttype':
        return ParameterType.ScriptType;
      case 'scriptbody':
        return ParameterType.ScriptBody;
      case 'assemblypath':
        return ParameterType.AssemblyPath;
      case 'classname':
        return ParameterType.ClassName;
      case 'methodname':
        return ParameterType.MethodName;
      default:
        return ParameterType.Constant;
    }
  }
