...
export function getSplitOnArrayAliasMetadata(schema: Swagger.Schema, required: boolean, propertyName?: string): SplitOnAliasMetadata {
  if (schema.type === SwaggerConstants.Types.Array) {
    return {
      alias: schema[SwaggerConstants.ExtensionProperties.Alias] as any,
      propertyName,
      required,
    };
...
export class ManifestParser {
  ...
  public getOutputParameters(
    ...
    const schemaProperties = new SchemaProcessor(schemaProcessorOptions).getSchemaProperties(
      selectedManifestOutputsSchema as Swagger.Schema
    );
