...
import type { WorkflowsCallbackInfo } from '../workflow';
import { isNumber ,
...
import type { ArmResources, BoundParameters, ContentLink, LogicAppsV2, Run, Runs } from '@microsoft/logic-apps-designer';
...
export class StandardRunService implements IRunService {
   ...
   * @param {WorkflowsCallbackInfo} callbackInfo - Information to call Api to trigger workflow.
   */
  async runTrigger(callbackInfo: WorkflowsCallbackInfo): Promise<void> {
    const { httpClient } = this.options;
