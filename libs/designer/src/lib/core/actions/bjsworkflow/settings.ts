...
/**
 * Determines whether trigger concurrency control is enabled in the persisted (saved) operation definition.
 * This reads the last-loaded workflow definition (not in-session edits), so it is only true once the
 * setting has been saved/published and reloaded. It is used to lock the trigger concurrency toggle while
 * still allowing unsaved (draft) changes to be reversed.
 */
