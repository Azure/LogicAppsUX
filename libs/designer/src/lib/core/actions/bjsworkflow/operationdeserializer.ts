...
import { initializeInputParameters, initializeOperationInfo, NodeParameters } from '../../state/operationMetadataSlice';
...
import * as ParametersHelper from '../../utils/parameterhelper';
...
const initializeOperationDetailsForManifest = async (
  ...
  const inputParameters = getInputParametersFromManifest(cachedOperationManifest as OperationManifest, definition);
  dispatch(initializeInputParameters({ id: nodeId, ...inputParameters }));
};
