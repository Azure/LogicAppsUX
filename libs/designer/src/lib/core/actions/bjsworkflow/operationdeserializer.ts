...
export const initializeOperationDetailsForManagedMcpServer = async (
  ...
  try {
    const referenceName = (<any>operation)?.inputs?.connectionReference?.connectionName || '';
    ...
    const mcpServerPath = (<any>operation).inputs?.parameters?.mcpServerPath
    if (!mcpServerPath) {
    ...
    const operationId = filteredOperations && filteredOperations.length > 0
    ...
    const builtinMcpServerOperationInfo = { connectorId: 'connectionProviders/mcpclient', operationId: 'nativemcpclient', type: 'McpClientTool', kind: 'Builtin'};
    ...
    const settings = getOperationSettings(/* isTrigger */ false, builtinMcpServerOperationInfo, builtinMcpServerManifest, undefined /* swagger */, operation, workflowKind);
