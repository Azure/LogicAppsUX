export default {
  manifest: {
    properties: {
      iconUri:
        'data:image/svg+xml;base64,',
      brandColor: '#8C6CFF',
      description: 'Specify the schema of JSON content.',
      summary: 'Parse JSON',
      inputs: {
        type: 'object',
        properties: {
          content: {
            title: 'Content',
            required: true,
            description: 'Content to create schema from',
          },
          schema: {
            type: 'object',
            title: 'Schema',
            description:
              'Example:\n{\n    "type": "object",\n    "properties": {\n        "address": {\n            "type": "string"\n        }\n    },\n    "required": ["address"]\n}',
            required: true,
            'x-ms-editor': 'schema',
          },
        },
        required: ['content', 'schema'],
      },
      inputsLocation: ['inputs'],
      isInputsOptional: false,
      outputs: {
        type: 'object',
        required: ['body'],
        properties: {
          body: {
            title: 'Body',
          },
        },
      },
      outputsSchema: {
        outputPaths: [
          {
            outputLocation: ['properties', 'body'],
            name: 'schema',
            schema: 'Value',
          },
        ],
      },
      isOutputsOptional: false,
      connector: {
        id: 'connectionProviders/dataOperationNew',
        name: 'dataOperationNew',
        properties: {
          description: 'Operations to work with data in workflow.',
          displayName: 'Data Operations',
        },
      },
      settings: {
        secureData: {
          options: {
            outputsMode: 'linkedtoinputs',
          },
        },
        trackedProperties: {
          scopes: ['action'],
        },
      },
    },
  },
  nodeParameters: undefined,
  operationMetadata: undefined,
  outputs: {
    body: {
      'category-actions': {
        name: 'Navigate To URL',
        type: 'navigateTo',
        value: 'Error! Hyperlink reference not valid.',
        foreground: true,
        authentication: true,
        destructive: false,
      },
    },
  },
  outputsParametersByName: {
    'outputs.$.body': {
      key: 'outputs.$.body',
      type: 'object',
      isAdvanced: false,
      name: 'body',
      title: 'Body',
      schema: {
        title: 'Body',
        type: 'object',
        properties: {
          'category-actions': {
            type: 'object',
            properties: {
              name: {
                type: 'string',
              },
              type: {
                type: 'string',
              },
              value: {
                type: 'string',
              },
              foreground: {
                type: 'boolean',
              },
              authentication: {
                type: 'boolean',
              },
              destructive: {
                type: 'boolean',
              },
            },
          },
        },
      },
      source: 'outputs',
      required: true,
    },
    'outputs.$.body.category-actions.name': {
      key: 'outputs.$.body.category-actions.name',
      type: 'string',
      isAdvanced: false,
      name: 'body.category-actions.name',
      title: 'Body name',
      schema: {
        type: 'string',
      },
      source: 'outputs',
      required: false,
    },
    'outputs.$.body.category-actions.type': {
      key: 'outputs.$.body.category-actions.type',
      type: 'string',
      isAdvanced: false,
      name: 'body.category-actions.type',
      title: 'Body type',
      schema: {
        type: 'string',
      },
      source: 'outputs',
      required: false,
    },
    'outputs.$.body.category-actions.value': {
      key: 'outputs.$.body.category-actions.value',
      type: 'string',
      isAdvanced: false,
      name: 'body.category-actions.value',
      title: 'Body value',
      schema: {
        type: 'string',
      },
      source: 'outputs',
      required: false,
    },
    'outputs.$.body.category-actions.foreground': {
      key: 'outputs.$.body.category-actions.foreground',
      type: 'boolean',
      isAdvanced: false,
      name: 'body.category-actions.foreground',
      title: 'Body foreground',
      schema: {
        type: 'boolean',
      },
      source: 'outputs',
      required: false,
    },
    'outputs.$.body.category-actions.authentication': {
      key: 'outputs.$.body.category-actions.authentication',
      type: 'boolean',
      isAdvanced: false,
      name: 'body.category-actions.authentication',
      title: 'Body authentication',
      schema: {
        type: 'boolean',
      },
      source: 'outputs',
      required: false,
    },
    'outputs.$.body.category-actions.destructive': {
      key: 'outputs.$.body.category-actions.destructive',
      type: 'boolean',
      isAdvanced: false,
      name: 'body.category-actions.destructive',
      title: 'Body destructive',
      schema: {
        type: 'boolean',
      },
      source: 'outputs',
      required: false,
    },
    'outputs.$.body.category-actions': {
      key: 'outputs.$.body.category-actions',
      type: 'object',
      isAdvanced: false,
      name: 'body.category-actions',
      title: 'Body category-actions',
      schema: {
        type: 'object',
        properties: {
          name: {
            type: 'string',
          },
          type: {
            type: 'string',
          },
          value: {
            type: 'string',
          },
          foreground: {
            type: 'boolean',
          },
          authentication: {
            type: 'boolean',
          },
          destructive: {
            type: 'boolean',
          },
        },
      },
      source: 'outputs',
      required: false,
    },
  },
};
