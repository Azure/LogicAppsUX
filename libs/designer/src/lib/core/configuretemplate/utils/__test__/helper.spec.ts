...
describe('ConfigureTemplateHelper', () => {
  describe('getParameterReferencesFromValue', () => {
    test('should return parameter name when value contains single parameter token', () => {
      const result = getParameterReferencesFromValue([
        { id: '1', type: 'token', token: { tokenType: 'parameter', name: 'test' } as any, value: "parameters('test')" },
      ]);
    ...
    test('should return all parameter references for complex fx tokens in value', () => {
      let result = getParameterReferencesFromValue([
        {
          id: '1',
          type: 'token',
          token: {
          ...
          value: "concat('abc', triggerBody(), parameters('param1))",
        },
      ]);
