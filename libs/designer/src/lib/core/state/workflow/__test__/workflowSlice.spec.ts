...
describe('workflowSlice - replaceOperationDefinition', () => {
  ...
  test('replaces the operation definition for an existing node', () => {
    const initialState = buildState({
      [nodeId]: { type: 'Compose', inputs: 'old-value' },
    });

    const newDefinition = { type: 'Compose', inputs: 'new-value', runAfter: {} } as unknown as LogicAppsV2.OperationDefinition;
    const newState = workflowSlice.reducer(
      initialState,
      workflowSlice.actions.replaceOperationDefinition({ nodeId, operationDefinition: newDefinition })
    );

    expect((newState.operations[nodeId] as any).inputs).toBe('new-value');
    expect((newState.operations[nodeId] as any).runAfter).toEqual({});
  });
  ...
  test('does nothing when the node does not exist', () => {
    ...
    const newState = workflowSlice.reducer(
      initialState,
      workflowSlice.actions.replaceOperationDefinition({
        nodeId: 'DoesNotExist',
        operationDefinition: { type: 'Compose', inputs: 'x' } as unknown as LogicAppsV2.OperationDefinition,
      })
    );

    expect(newState.operations['DoesNotExist']).toBeUndefined();
    expect((newState.operations[nodeId] as any).inputs).toBe('old-value');
  });
});
