...
describe('getWorkflowNameFromOperation', () => {
  ...
  describe('unique names based on existing workflows', () => {
    it('should return name based on existing workflows', () => {
      const result = getWorkflowNameFromOperation('Send Email', 'send-email-action', ['Get_Email']);
      expect(result).toBe('Send_Email');
    });

    it('should return a unique name based on existing workflows', () => {
      const result = getWorkflowNameFromOperation('Send Email', 'send-email-action', ['Send_Email']);
      expect(result).toBe('Send_Email_1');
    });

    it('should check for all existing workflows and return a unique name', () => {
      const result = getWorkflowNameFromOperation('Send Email', 'send-email-action', ['Send_Email', 'Send_Email_1', 'Send_Email_2']);
      expect(result).toBe('Send_Email_3');
    });
  });
