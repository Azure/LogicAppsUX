...
export const getListDynamicValues = async (
    ...
    return queryClient.fetchQuery(['listdynamicvalues', { connectionId }, { connectorId }, { operationId }, { parameter: getParametersKey(parameters)}], () =>
...
export const getDynamicSchemaProperties = async (
    ...
    return queryClient.fetchQuery(['dynamicschemaproperties', { connectionId }, { connectorId }, { operationId }, { parameter: getParametersKey(parameters)}], () =>
...
const getParametersKey = (parameters: Record<string, any>): string => {
    return Object.keys(parameters).reduce((result: string, parameterKey: string) => (`${result}, ${parameterKey}-${parameters[parameterKey]}`), '');
}