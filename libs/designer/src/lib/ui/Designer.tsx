...
import { useWindowDimensions, WORKFLOW_NODE_TYPES, useThrottledEffect, isString } from '@microsoft/utils-logic-apps';
...
export const Designer = (props: DesignerProps) => {
  ...
  const isFetchingInitialData = useIsFetching({
    predicate: (query) => {
      ...
      const queryKeyArray = query.queryKey && isString(query.queryKey) ? [query.queryKey] : (query.queryKey as string[]);
      return queryKeyArray.some((val) => queryKeysToWatch.includes(val));
    },
  });
