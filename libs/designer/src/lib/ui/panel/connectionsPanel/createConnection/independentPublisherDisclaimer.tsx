 ...
 */
export const IndependentPublisherDisclaimer = ({ createButtonText }: IndependentPublisherDisclaimerProps) => {
  const intl = useIntl();
  ...
  const disclaimerText = intl.formatMessage({
    ...
    id: 'hKyjZ7',
    description: 'Disclaimer text for Independent Publisher connectors warning about risks',
  });

  const learnMoreText = intl.formatMessage({
    defaultMessage: 'Learn more',
    id: 'ec13/p',
    description: 'Link text to learn more about Independent Publisher connectors',
  });
  ...
  return (
    <div className={styles.disclaimerContainer}>
      <Text className={styles.disclaimerText}>
        {disclaimerText}{' '}
        <Link href={CERTIFICATION_DOCS_URL} target="_blank" rel="noopener noreferrer" inline>
          {learnMoreText}
        </Link>
      </Text>
      <Text className={styles.licenseText}>
        {bySelectingText} <span className={styles.boldText}>{buttonText}</span> {youAgreeToTheText}{' '}
        <Link href={MIT_LICENSE_URL} target="_blank" rel="noopener noreferrer" inline>
          MIT License
        </Link>
        .
      </Text>
