...
const DefaultNode = ({ id }: NodeProps) => {
  ...
  }, [metadata, parentActionMetadata, isMonitoringView])
  ...
  const actualToolRunIndex = useMemo(() => metadata?.subgraphType === SUBGRAPH_TYPES.MCP_CLIENT ? mcpToolRunIndex : toolRunIndex, [metadata, toolRunIndex, mcpToolRunIndex]);
  ...
  const isAgentTool = useMemo(() => metadata?.subgraphType === SUBGRAPH_TYPES.MCP_CLIENT, [metadata?.subgraphType]);
