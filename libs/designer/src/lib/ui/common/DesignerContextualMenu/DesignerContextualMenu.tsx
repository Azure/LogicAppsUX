...
export const DesignerContextualMenu = () => {
  ...
  const subgraphMenuItems: JSX.Element[] = useMemo(
    () => [
      ...(metadata?.subgraphType === SUBGRAPH_TYPES.SWITCH_CASE || metadata?.subgraphType === SUBGRAPH_TYPES.AGENT_CONDITION || metadata?.subgraphType === SUBGRAPH_TYPES.AGENT_TOOL
  ...
  const menuItems = useMemo(() => {
    ...
    if ((metadata?.subgraphType && metadata?.subgraphType !== SUBGRAPH_TYPES.AGENT_TOOL) || isScopeNode) {
