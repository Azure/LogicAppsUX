...
export const LoopsPager = ({ metadata, scopeId, collapsed, focusElement, useToolRun }: LoopsPagerProps) => {
  ...
  const currentPage = useMemo(() => useToolRun ? nodeMetadata?.toolRunIndex : nodeMetadata?.runIndex ?? 0, [nodeMetadata]);
  const loopsCount = useMemo(() => getLoopsCount(useToolRun ? metadata?.toolRunData : metadata?.runData), [metadata?.runData]);
