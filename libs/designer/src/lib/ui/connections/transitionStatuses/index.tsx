...
export interface TransitionStatusesIndicatorProps {
  transition: LogicAppsV2.Transition;
  sourceId: string;
  targetId: string;
}

export interface CollapsedTranistionStatusesIndicatorProps {
  filteredTransitions: LogicAppsV2.Transitions;
  transitionCount: number;
}

export function TransitionStatusesIndicator({ transition, targetId }: TransitionStatusesIndicatorProps): JSX.Element {
  const intl = useIntl();
  const { isInverted } = useTheme();
  const themeName = isInverted ? 'dark' : 'light';
  const normalizedStatuses = (transition?.when ?? []).map((status) => status.toUpperCase()) as RUN_AFTER_STATUS[];
  ...
  const tooltipHeaderText = intl.formatMessage(
    {
      defaultMessage: 'Transition to {sourceNodeId}',
      id: 'ohid3T',
      description: 'The text that shows the transition to another node.',
    },
    {
      sourceNodeId: <span style={{ fontWeight: 'bold' }}>{idDisplayCase(targetId)}</span>,
    }
  );
...
export function CollapsedTransitionStatusesIndicator({
  filteredTransitions,
  transitionCount,
}: CollapsedTranistionStatusesIndicatorProps): JSX.Element {
  const intl = useIntl();
  ...
  Object.entries(filteredTransitions).forEach(([source, transition], index) => {
    const normalizedStatuses = (transition?.when ?? []).map((status) => status.toUpperCase()) as RUN_AFTER_STATUS[];
    ...
    const tooltipContent = (
      <div className="msla-run-after-tooltip-container">
        ...
        {index !== transitionCount - 1 && <Divider />}
      </div>
    );
  ...
  return (
    <Tooltip relationship={'description'} withArrow content={<div className="msla-run-after-content">{tooltipContents}</div>}>
      <span className="msla-run-after-overflow">&times;{transitionCount}</span>
    </Tooltip>
  );
}
