...
function GeneralSettings({
  ...
  // Trigger concurrency control is irreversible once saved: lock the toggle when it is enabled in the
  // persisted (last-loaded) workflow definition. Unsaved (draft) enablement stays reversible because
  // state.workflow.operations reflects the saved definition, not in-session setting edits. The lookup
  // prefers the current id and falls back to the original id so a renamed trigger stays resolved both
  // before and after a same-session save/reinitialize.
