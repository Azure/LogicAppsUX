const iconUri =
  'data:image/svg+xml;base64,';

const brandColor = '#59B2D9';

export const selectManualWorkflowGroup = {
  id: 'connectionProviders/workflow',
  name: 'connectionProviders/workflow',
  properties: {
    displayName: 'Azure Logic Apps',
    description: 'Azure Logic Apps',
    iconUri,
    brandColor,
    capabilities: ['actions'],
  },
};

export const selectManualWorkflowOperation = {
  id: 'selectmanualworkflow',
  name: 'selectmanualworkflow',
  type: 'selectmanualworkflow',
  properties: {
    api: {
      id: 'connectionProviders/workflow',
      name: 'connectionProviders/workflow',
      displayName: 'Azure Logic Apps',
      iconUri,
      brandColor,
      description: 'Azure Logic Apps',
    },
    summary: 'Choose a Logic Apps workflow',
    description: 'Show Logic Apps in the same region',
    visibility: 'Important',
    operationType: 'workflow',
    brandColor,
    iconUri,
  },
};
