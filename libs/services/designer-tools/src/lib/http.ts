...
export class HttpClient {
    ...
    private _parseRequest<TRequestBody>(httpMethod: string, request: HttpRequest<TRequestBody>): ParsedRequest {
        ...
        url = this._getFullPathUrl(url, request.query);
    ...
    private _getFullPathUrl( path: string, query: QueryRecord | undefined): string {
    ...
    /**
     * Removes starting and trailing '/' from the url
     */
    ...
    private _getResponseBody<T>(request: HttpRequest<T>, response: Response): Promise<any> {
        return response.text().then(responseText => {
            if (request.json) {
                // NOTE(gcorvera): Some requests return no body, in that case we shouldn't try to
                // parse into JSON, we should return a null object as the response will be ignored.
                // attempting to parse "" with JSON.parse or Body.json() throws an exception
    ...
    private _setClientRequestIdInResponse(responseHeaders: Headers, clientRequestId: string): Headers {
        if (responseHeaders instanceof Headers && !responseHeaders.has(X_MS_CLIENT_REQUEST_ID)) {
            const headers = new Headers(responseHeaders);

            headers.set(X_MS_CLIENT_REQUEST_ID, clientRequestId);
