...
export const ResourceSelector = (props: ResourceSelectorProps) => {
  ...
  const resources = useMemo(
    () => ((itemsQuery?.data ?? []) as any[]).sort((a, b) => a.name.localeCompare(b.name)),
    [itemsQuery.data]
  );
    ...
    <div>
        ...
        <div className="msla-function-apps-container">
            <div className="msla-function-app-list-header">
                {headers.map(header => (<Text key={header}>{header}</Text>))}
            </div>
...
return (
    ...
    >
        {getColumns(resource).map(value => (<Text key={value}>{value}</Text>))}
    </button>
