...
describe('HttpClient', () => {
  ...
  describe('auth token handling', () => {
    ...
    it('should update token via updateAccessToken and use new token on next request', async () => {
      const clientToUpdate = new HttpClient({ ...httpClientOptions, accessToken: 'old-token' });
      clientToUpdate.updateAccessToken('new-token');
      (axios as any).mockResolvedValue({ data: {}, status: 200 });

      await clientToUpdate.get({ uri: '/subscriptions/sub-1/resource', headers: {} });
