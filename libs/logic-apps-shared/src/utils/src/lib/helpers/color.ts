...
export const rgb2hsv = ({ red, green, blue }: RGB): HSV => {
...
export const lighten = ({ blue, green, red }: RGB, amount: number): RGB => {
...
export function getContrastTextColor(hexColor?: string): string | undefined {
  if (!hexColor) {
    return;
  }
