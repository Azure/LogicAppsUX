...
import type { Parameters } from '../models/logicAppsV2';
...
export const getUniqueName = (keys: string[], prefix: string, delimiter = '-'): { name: string; index: number } => {
  const set = new Set(keys.map((name) => name.split('::')[0].toLowerCase()));

  let index = 0;
  let name = prefix;
  while (set.has(name.toLowerCase())) {
    name = `${prefix}${delimiter}${++index}`;
  }
