...
export const getAllConnectorProperties = (
  connector: Connector | OperationApi | undefined,
): Partial<ConnectorProperty & OperationApi> => {
  if (!connector) {
    return {};
  }
...
export const getBrandColorFromConnector = (
  connector: Connector | OperationApi | undefined,
): string => {
  const connectorData = getAllConnectorProperties(connector);
  return (
    connectorData?.brandColor ?? connectorData.metadata?.brandColor ?? "#000000"
  );
};

export const getDescriptionFromConnector = (
  connector: Connector | OperationApi | undefined,
): string => {
  const connectorData = getAllConnectorProperties(connector);
  return (
    connectorData.description ??
    connectorData.generalInformation?.description ??
    ""
  );
};

export const getDisplayNameFromConnector = (
  connector: Connector | OperationApi | undefined,
): string => {
  return getAllConnectorProperties(connector)?.displayName ?? "";
};

export const getIconUriFromConnector = (
  connector: Connector | OperationApi | undefined,
): string => {
  const connectorData = getAllConnectorProperties(connector);
  const iconUrl =
    connectorData.iconUrl ??
    connectorData.iconUri ??
    connectorData.generalInformation?.iconUrl;
...
export function normalizeConnectorId(
  connectorId: string,
  ...
  const result = isArmResourceId(connectorId)
    ? connectorId
        .replaceAll(subscriptionPlaceholder, subscriptionId)
        .replaceAll(locationPlaceholder, location)
    : connectorId;
...
export function normalizeConnectorIds(
  ...
  return connectorIds.map((connectorId) =>
    normalizeConnectorId(connectorId, subscriptionId, location, lowercase),
  );
}
 ...
 */
export const isIndependentPublisherConnector = (
  connector: Connector | OperationApi | undefined,
): boolean => {
  if (!connector) {
    return false;
