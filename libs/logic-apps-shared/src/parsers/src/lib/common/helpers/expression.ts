import type { ParserExpression, ExpressionFunction, ExpressionLiteral, ExpressionStringInterpolation } from '../../models/expression';
...
export function isStringLiteral(expression: ParserExpression): expression is ExpressionLiteral {
  return equals(expression.type, ExpressionType.StringLiteral);
}

export function isLiteralExpression(expression: ParserExpression): expression is ExpressionLiteral {
  return isStringLiteral(expression) || isNumberLiteral(expression) || isBooleanLiteral(expression) || isNullLiteral(expression);
}

export function isFunction(expression: ParserExpression): expression is ExpressionFunction {
  return equals(expression.type, ExpressionType.Function);
}

export function isStringInterpolation(expression: ParserExpression): expression is ExpressionStringInterpolation {
...
function isNullLiteral(expression: ParserExpression): boolean {
  return equals(expression.type, ExpressionType.NullLiteral);
}

function isBooleanLiteral(expression: ParserExpression): boolean {
  return equals(expression.type, ExpressionType.BooleanLiteral);
}

function isNumberLiteral(expression: ParserExpression): boolean {
