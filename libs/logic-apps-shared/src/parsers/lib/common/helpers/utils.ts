...
export function toSwaggerSchema(
  parameter: Parameter | OpenAPIV2.ItemsObject
): SchemaObject {
...
export function getEnum(
  ...
  if (parameter.enum) {
    const customEnum: EnumObject[] =
      parameter[Constants.ExtensionProperties.CustomEnum];
    if (customEnum) {
      return customEnum;
    } else {
      const editorOptions =
        parameter[Constants.ExtensionProperties.EditorOptions];
...
export function getParameterDynamicValues(
  parameter: SchemaObject
): ParameterDynamicValues | undefined {
  const parameterDynamicListExtension =
    parameter[Constants.ExtensionProperties.DynamicList] ??
    parameter?.items?.[Constants.ExtensionProperties.DynamicList];
  const parameterDynamicValuesExtension =
    parameter[Constants.ExtensionProperties.DynamicValues] ??
    parameter?.items?.[Constants.ExtensionProperties.DynamicValues];
  const parameterDynamicTreeExtension =
    parameter[Constants.ExtensionProperties.DynamicTree] ??
    parameter?.items?.[Constants.ExtensionProperties.DynamicTree];
...
export function getParameterDynamicSchema(
  parameter: SchemaObject
): ParameterDynamicSchema | undefined {
  const dynamicSchemaExtension =
    parameter[Constants.ExtensionProperties.DynamicSchema];
  ...
  const dynamicPropertiesExtension =
    parameter[Constants.ExtensionProperties.DynamicProperties];
  ...
  if (schema.type === "array") {
    ...
    const keys = Object.keys(properties).filter((key) => {
      ...
      return !(
        excludeInternal &&
        equals(
          property[Constants.ExtensionProperties.Visibility],
    ...
    if (keys.length === 1) {
      ...
      const propertyRequired =
        required && (schema.required || []).indexOf(firstKey) !== -1;
...
export function getEditorForParameter(
  ...
  if (
    isLegacyDynamicValuesTreeExtension(dynamicValues) ||
 ...
 * @return {SchemaObject}
 */
export function dereferenceRefSchema(
  $ref: string,
  ...
  const cyclicalDefinitions: Record<string, any> = Object.keys($refs).reduce(
    makeDefinition($refs),
    {}
  );
  ...
  switch (name) {
    ...
    case OutputKeys.PathParameters:
      return intl.formatMessage({
        ...
        id: "BJNUxN",
        description:
          "Display name for relative path parameters in trigger outputs",
      });
    default:
