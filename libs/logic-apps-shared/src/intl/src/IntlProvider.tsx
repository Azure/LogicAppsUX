...
const loadLocaleData = async (locale: string): Promise<Record<string, string> | Record<string, MessageFormatElement[]>> => {
  ...
  switch (locale.split('-')[0].toLowerCase()) {
    ...
    case 'pt': {
      if (locale === 'pt-BR') {
        messages = await import('./compiled-lang/strings.pt-BR.json');
      } else {
        messages = await import('./compiled-lang/strings.pt-PT.json');
      }
      break;
    }
    case 'ru': {
    ...
    case 'zh': {
      if (locale === 'zh-Hans') {
        messages = await import('./compiled-lang/strings.zh-Hans.json');
      } else {
        messages = await import('./compiled-lang/strings.zh-Hant.json');
      }
      break;
    }
    case 'fr': {
    ...
    case 'en': {
      if (locale === 'en-XA') {
        messages = await import('./compiled-lang/strings.en-XA.json');
      } else {
