...
export class StandardRunService implements IRunService {
   ...
   */
  async getActionLinks(actionMetadata: { inputsLink?: ContentLink; outputsLink?: ContentLink }, nodeId: string, doNotParse = false): Promise<any> {
    ...
    return { inputs: doNotParse ? inputs : this.parseActionLink(inputs, true), outputs: doNotParse ? outputs : this.parseActionLink(outputs, false) };
  }
