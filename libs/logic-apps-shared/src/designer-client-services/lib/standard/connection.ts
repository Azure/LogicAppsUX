  ...
  private async _getConnectionsConfiguration(
    ...
    switch (connectionType) {
      ...
      case ConnectionType.Mcp: {
        connectionsData = convertToMcpConnectionsData(connectionName, connectionInfo, parametersMetadata, connector.id);
...
function convertMcpConnectionDataToConnection(connectionKey: string, connectionData: AgentMcpConnectionModel): Connection {
  ...
  const authentication = connectionData.authentication ?? {};
...
function convertToMcpConnectionsData(
  ...
  const settings: any = {};
  const { displayName, connectionParametersSet, mode } = connectionInfo;
  let authentication: any = null;

  const processValue = function (values: Record<string, { value: any }> | undefined, key: string, inSetting: boolean): any {
