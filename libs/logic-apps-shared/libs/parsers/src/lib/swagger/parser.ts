...
import { ChunkCapabilities, ExtensionProperties, PropertyName, Visibility } from '../common/constants';
...
export class SwaggerParser {
  ...
  private _getUploadChunkMetadata(capabilities: Record<string, any>): UploadChunkMetadata {
    return {
      chunkTransferSupported: !!capabilities && !!capabilities[ChunkCapabilities.ChunkTransfer],
      acceptUploadSize: !!capabilities && !!capabilities[ChunkCapabilities.AcceptUploadChunkSize],
      minimumSize: capabilities ? capabilities[ChunkCapabilities.MinimumUploadChunkSize] : undefined,
      maximumSize: capabilities ? capabilities[ChunkCapabilities.MaximumUploadChunkSize] : undefined,
    };
  }

  private _getDownloadChunkMetadata(capabilities: Record<string, any>): DownloadChunkMetadata {
    return {
      acceptDownloadSize: !!capabilities && !!capabilities[ChunkCapabilities.AcceptDownloadChunkSize],
      minimumSize: capabilities ? capabilities[ChunkCapabilities.MinimumDownloadChunkSize] : undefined,
      maximumSize: capabilities ? capabilities[ChunkCapabilities.MaximumDownloadChunkSize] : undefined,
    };
  }
